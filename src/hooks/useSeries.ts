...
const useSeries = () => {
  ...
  const createSeries = useCallback(
    async (seriesData: CreateSeriesData) => {
      try {
        setIsLoading(true);
        const response = await fetch("/api/series", {
          method: "POST",
  ...
  const updateSeries = useCallback(
    async (id: string, updates: UpdateSeriesData) => {
      try {
        setIsLoading(true);
  ...
  // Seriye hikaye ekle
  const addStoryToSeries = useCallback(
    async (seriesId: string, storyId: string, seriesTitle: string) => {
      try {
        setIsLoading(true);
