...
export function useStoryDatabase(): UseStoryDatabaseReturn {
  ...
  // Masal güncelle
  const updateStory = async (id: string | number, storyText: string, storyType: string, customTopic: string | null = null): Promise<Story> => {
    setLoading(true)
    setError(null)
    try {
      const updatedStory = await databaseService.updateStory(String(id), storyText, storyType, customTopic)
      await loadStories() // Listeyi yenile
      return updatedStory
    } catch (err: unknown) {
      const errorMessage = err instanceof Error ? err.message : 'Bilinmeyen hata'
