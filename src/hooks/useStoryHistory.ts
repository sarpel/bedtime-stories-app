...
export function useStoryHistory() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error(
        "Masal geçmişi yüklenirken hata:",
  ...
  const addToHistory = (story: Story): number => {
    ...
    setHistory((prev) => {
      ...
      const existingIndex = prev.findIndex(
        (item) =>
          item.story === story.story && item.storyType === story.storyType,
      );
  ...
  const removeFromHistory = (id: number): void => {
    ...
    setHistory((prev) => {
      console.log(
        `[useStoryHistory] History before filter: ${prev.length} items`,
      );
      const updatedHistory = prev.filter((item) => item.id !== id);
      console.log(
        `[useStoryHistory] History after filter: ${updatedHistory.length} items`,
      );
  ...
  const updateStoryAudio = (id: number, audioUrl: string): void => {
    setHistory((prev) =>
      prev.map((item) =>
        item.id === id ? { ...item, audioUrl, audioGenerated: true } : item,
      ),
    );
  };
