    ...
    <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-0 sm:p-4">
      <div ref={panelRef} className="w-full h-full sm:w-auto sm:h-auto">
        <Card className="w-full h-full sm:w-[600px] sm:h-[500px] overflow-y-auto scrollbar-thin border-0 sm:border rounded-none sm:rounded-lg">
          <CardHeader className="pb-3 sm:pb-4 p-3 sm:p-6 sticky top-0 bg-card/95 backdrop-blur-sm border-b z-10">
          ...
          <CardContent className="space-y-3 sm:space-y-4 p-3 sm:p-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {searchQuery && (
                    <Badge variant="outline">
                      "{searchQuery}" için{" "}
                      {searchType === "all" ? "tüm" : searchType} arama
                    </Badge>
                  )}
                </div>
                ...
                {enrichedResults.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p className="text-sm mt-1">
                      Farklı kelimeler deneyin veya arama türünü değiştirin.
                    </p>
                  </div>
                ) : (
            ...
            {!searchQuery && !hasSearched && searchHistory.length === 0 && (
              <div className="text-center py-8 text-muted-foreground">
                <Search className="h-12 w-12 mx-auto mb-4 opacity-50" />
                <p>
                  Masallarınızda arama yapmak için yukarıdaki kutucuğa yazın.
                </p>
                <p className="text-sm mt-1">
