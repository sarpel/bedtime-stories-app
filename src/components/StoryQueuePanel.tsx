...
function SortableStoryItem({
  ...
  return (
    <div
      ref={setNodeRef}
      className={`flex flex-col sm:flex-row items-start sm:items-center justify-between p-1 border rounded-lg transition-colors min-h-12 sm:h-12 gap-1 ${
        isCurrent
    ...
    >
      ...
      <div className="flex-1 min-w-0 w-full sm:w-auto">
        <div className="flex flex-wrap sm:flex-nowrap items-center gap-0.5 mb-0.5 sm:mb-0.5">
          ...
          <Badge
            variant="secondary"
            className="text-[11px] sm:text-[9px] px-1.5 sm:px-1 py-0.5 h-4 sm:h-3 leading-none"
          >
          ...
          {(story.custom_topic || story.customTopic) && (
            <Badge
              variant="outline"
              className="text-[11px] sm:text-[9px] px-1.5 sm:px-1 py-0.5 h-4 sm:h-3 leading-none max-w-24 truncate"
            >
              {story.custom_topic || story.customTopic}
            </Badge>
          )}
      ...
      <div className="flex items-center gap-1 sm:gap-0.5 ml-0 sm:ml-2 shrink-0 w-full sm:w-auto justify-end">
        ...
        {(story.audio || story.audioUrl) && (
          <div className="flex items-center">
            <AudioControls
              ...
              currentStoryId={
                audioCurrentStoryId ? String(audioCurrentStoryId) : null
              }
              onPlay={playAudio}
        ...
        {(story.audio || story.audioUrl) && (
          <Button
            ...
            className={`h-5 w-5 p-0 ${
              remoteStatus.playing && remoteStatus.storyId === story.id
                ? "text-primary"
                : ""
            }`}
...
export default function StoryQueuePanel({
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (active.id !== over.id) {
      ...
      // DB senkron
      try {
        queueService.setQueueIds(
          newQueue.map((s) => String(s.id!)).filter(Boolean)
        );
      } catch {
        /* queue sync optional */
      }
  ...
  useEffect(() => {
    setQueue((prev) => {
      ...
      const updated = prev.map(
        (item) => (localStories || []).find((s) => s.id === item.id) || item
      );
      ...
      if (prev.length === updated.length) {
        ...
        for (let i = 0; i < prev.length; i++) {
          ...
          if (
            p.id !== u.id ||
            pf !== uf ||
            (p.story_text || p.story || "") !==
              (u.story_text || u.story || "") ||
            (p.custom_topic || p.customTopic || "") !==
              (u.custom_topic || u.customTopic || "")
          ) {
            same = false;
            break;
          }
  ...
  const playAtIndex = useCallback(
    (idx: number) => {
      ...
      const audioUrl = item.audio
        ? getDbAudioUrl(item.audio.file_name)
        : item.audioUrl;
      if (!audioUrl) {
  ...
  const addToQueue = (story: Story) => {
    ...
    try {
      if (story.id) queueService.add(String(story.id));
    } catch {
      /* queue sync optional */
    }
  };
          ...
          <div className="flex flex-wrap gap-1 sm:gap-2 w-full sm:w-auto">
            ...
            >
              ...
              <span className="ml-1 hidden sm:inline">
                {remoteStatus.playing ? "Dur" : "Oynat"}
              </span>
            </Button>
            <Button
              variant="outline"
            ...
            <Button
              variant={repeatAll ? "default" : "outline"}
              size="sm"
            ...
            >
              ...
              <span className="ml-1 hidden sm:inline">Temizle</span>
            </Button>
            <Button
              variant="outline"
          ...
          <DialogContent>
            ...
            <div className="space-y-2">
              ...
              <div className="text-xs text-muted-foreground">
                Kütüphanedeki masallar listelenir. Zaten kuyrukta olanlar
                gizlenir.
              </div>
                    ...
                    >
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            {getStoryTypeLabel(s.story_type || s.storyType)}
                          </Badge>
                        ...
                        <div className="text-xs text-muted-foreground truncate max-w-[36ch]">
                          {(s.story_text || s.story).slice(0, 100)}...
                        </div>
                      </div>
                      <div className="shrink-0 flex items-center gap-2">
                        {s.audio || s.audioUrl ? (
                          <Badge
                            variant="default"
                            className="text-[10px] bg-emerald-600 text-white hover:bg-emerald-600/90"
                          >
                            Ses Var
                          </Badge>
                        ) : (
                        ...
                        <Button size="sm" onClick={() => addToQueue(s)}>
                          Ekle
                        </Button>
                      </div>
                    </div>
          ...
          <DialogContent className="sm:max-w-lg">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Örn: prenses ve ejderha"
              />
            ...
            <DialogFooter>
              ...
              <Button
                onClick={async () => {
                  if (!editTarget || !editTarget.id || !onUpdateStory) {
                    setEditTarget(null);
                    return;
                  }
                  try {
