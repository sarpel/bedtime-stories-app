...
export default function AnalyticsDashboard({ onClose }: AnalyticsDashboardProps) {
  ...
  const [storyTypePopularity, setStoryTypePopularity] = useState<StoryTypePopularity[]>([])
          ...
          <Card className="w-[600px] h-[500px] overflow-y-auto scrollbar-thin">
            ...
            <CardContent className="p-3">
              <div className="text-center py-8">
                ...
                <p className="text-muted-foreground text-sm">Henüz analitik veri yok</p>
              </div>
            </CardContent>
          </Card>
        </div>
  ...
  return (
    <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-1">
      <div ref={panelRef}>
        ...
        <CardContent className="p-2">{/* Son güncelleme bilgisi kaldırıldı - yer tasarrufu */}
          <Tabs defaultValue="overview" className="space-y-2">
            ...
            <TabsContent value="overview" className="space-y-2">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
                <Card className="p-2">
                  <div className="text-xs font-medium mb-1">Masal Başarı: {overview.story.successRate}%</div>
                ...
                <Card className="p-2">
                  <div className="text-xs font-medium mb-1">Ses Başarı: {overview.audio.audioSuccessRate}%</div>
                  <Progress value={overview.audio.audioSuccessRate} className="h-1" />
                  <div className="flex justify-between text-xs text-muted-foreground mt-1">
                    <span>✓{overview.audio.successfulGenerated || 0}</span>
                    <span>✗{overview.audio.totalGenerated - (overview.audio.successfulGenerated || 0)}</span>
                  </div>
                </Card>
              </div>
            </TabsContent>
            ...
            <TabsContent value="stories" className="space-y-2">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
                <Card className="p-2">
                  <div className="text-xs font-medium mb-2">Popüler Türler</div>
                  <div className="space-y-1">
                    {storyTypePopularity.length > 0 ? (
                      storyTypePopularity.slice(0, 5).map((item, index) => (
                        <div key={item.storyType} className="flex items-center justify-between text-xs">
                          <div className="flex items-center gap-1">
                            <Badge variant="outline" className="text-xs px-1 py-0">#{index + 1}</Badge>
                            <span className="truncate">{getStoryTypeName(item.storyType)}</span>
                          </div>
                          <div className="flex items-center gap-1">
                    ...
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="font-medium">
                          {overview.story.avgDuration && overview.story.avgDuration > 0 ? `${(overview.story.avgDuration / 1000).toFixed(1)}s` : 'N/A'}
                        </span>
                      </div>
                    </div>
                    <div className="flex items-center justify-between text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                <Card className="p-2">
                  ...
                  <div className="flex justify-between text-xs">
                    <span className="text-green-600">✓{overview.audio.successfulGenerated}</span>
                    <span className="text-red-600">✗{overview.audio.totalGenerated - (overview.audio.successfulGenerated || 0)}</span>
                  </div>
                </Card>
            ...
            <TabsContent value="errors" className="space-y-2">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
                <Card className="p-2">
                  ...
                  <div className="text-lg font-bold text-red-600">{overview.errors?.totalErrors || 0}</div>
                  <div className="text-xs text-muted-foreground">
                ...
                <Card className="p-2">
                  ...
                  <div className="space-y-1">
                    {overview.errors?.errorTypes && overview.errors.errorTypes.length > 0 ? (
                      ...
                      <p className="text-xs text-muted-foreground text-center py-2">Hata yok! 🎉</p>
                    )}
                  </div>
                </Card>
              </div>
              ...
              {overview.errors?.totalErrors === 0 && (
                <Card className="p-3">
                  <div className="text-center">
                    ...
                    <div className="text-xs font-medium">Harika! Hiç hata yok</div>
                  </div>
                </Card>
              )}
            </TabsContent>
