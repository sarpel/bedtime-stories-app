    ...
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 mb-0.5">
          <Badge
            variant="secondary"
            className="text-[10px] px-1.5 py-0.5 h-4 leading-none"
          >
...
export const OptimizedStoryList = memo(
  ({
    ...
    const { displayedItems, isLoading, hasMore, loadMore } =
      useProgressiveLoading(
        (isExpanded
          ? filteredStories
          : filteredStories.slice(0, initialDisplayCount)) as any,
        batchSize,
      );
    ...
    return (
      <div className="space-y-4">
        ...
        <div className={`${maxHeight} overflow-y-auto space-y-1`}>
          {displayedItems.length === 0 ? (
            ...
            <>
              ...
              {!isExpanded && filteredStories.length > initialDisplayCount && (
                <div className="text-center py-2">
                  ...
                  >
                    ...
                    {filteredStories.length - displayedItems.length} masal daha
                    - Tümünü Görüntüle
                  </Button>
                </div>
              )}
              ...
              {isExpanded &&
                !hasMore &&
                filteredStories.length > initialDisplayCount && (
                  <div className="text-center py-2">
                    <Button
                      variant="ghost"
                      size="sm"
        ...
        <div className="text-xs text-muted-foreground text-center">
          Toplam: {stories.length} | Filtrelenen: {filteredStories.length} |
          Gösterilen: {displayedItems.length}
        </div>
      </div>
    );
  },
