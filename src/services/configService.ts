...
export const config = {
  ...
  openai: {
    ...
    endpoint: isProduction
      ? "/api/llm"
      : import.meta.env.VITE_OPENAI_ENDPOINT || "/api/llm",
  },
  ...
  elevenlabs: {
    ...
    endpoint: isProduction
      ? "/api/tts"
      : import.meta.env.VITE_ELEVENLABS_ENDPOINT || "/api/tts",
  },
  ...
  geminiLLM: {
    ...
    endpoint: isProduction
      ? "/api/llm"
      : import.meta.env.VITE_GEMINI_LLM_ENDPOINT || "/api/llm",
  },
  ...
  geminiTTS: {
    ...
    endpoint: isProduction
      ? "/api/tts"
      : import.meta.env.VITE_GEMINI_TTS_ENDPOINT || "/api/tts",
  },
...
export const validateConfig = () => {
  ...
  if (!isProduction) {
    if (!config.openai.apiKey) {
      errors.push(
        "OpenAI API anahtarı eksik (.env dosyasında VITE_OPENAI_API_KEY)",
      );
    }

    if (!config.elevenlabs.apiKey) {
      errors.push(
        "ElevenLabs API anahtarı eksik (.env dosyasında VITE_ELEVENLABS_API_KEY)",
      );
    }
  }
