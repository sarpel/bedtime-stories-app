...
class OptimizedDatabaseService {
  ...
  async cachedFetch(
    ...
    const requestPromise = fetch(url, {
      ...
      .then(async (response) => {
        ...
        console.log("[DB] cachedFetch:response", {
          url,
  ...
  async createStory(
    ...
    console.log(
      "storyText:",
      typeof storyText,
      storyText ? storyText.substring(0, 100) + "..." : "null/undefined",
    );
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Story creation failed: ${response.status} - ${errorText}`,
      );
    }
  ...
  async deleteStory(id: string) {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error("[DB] deleteStory:fetch_error_response", {
          status: response.status,
          errorText,
        });
        throw new Error(
          `Story deletion failed: ${response.status} - ${errorText}`,
        );
      }
      ...
      console.error("[DB] deleteStory:fetch_exception", {
  ...
  async migrateFromLocalStorage() {
    try {
      ...
      console.log(
        `${stories.length} masal localStorage'dan veritabanına taşınacak...`,
      );
      ...
      for (const story of stories) {
        try {
          ...
          const exists = existingStories.some(
            (existing: Story) =>
