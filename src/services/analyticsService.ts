...
class AnalyticsService {
  ...
  trackStoryGeneration(storyType: string | null, customTopic: string | null, success: boolean, duration: number, error: string | null = null): void {
  ...
  trackAudioGeneration(storyId: string, voiceId: string, success: boolean, duration: number, error: string | null = null): void {
    const event: AnalyticsEvent = {
  ...
  trackAudioPlayback(storyId: string, action: string, position: number = 0): void {
    const event: AnalyticsEvent = {
      type: 'audio_playback',
      timestamp: Date.now(),
      sessionId: this.sessionId,
  ...
  async getStoryGenerationMetrics(timeRange: string = '7d'): Promise<any> {
    try {
      ...
      const successful = events.filter((e: AnalyticsEvent) => e.data.success).length
      const failed = total - successful
      const successRate = total > 0 ? (successful / total) * 100 : 0

      // Average generation time
      const successfulEvents = events.filter((e: AnalyticsEvent) => e.data.success && e.data.duration > 0)
      const avgDuration = successfulEvents.length > 0
        ? successfulEvents.reduce((sum: number, e: AnalyticsEvent) => sum + e.data.duration, 0) / successfulEvents.length
        : 0
  ...
  async getAudioMetrics(timeRange: string = '7d'): Promise<any> {
    try {
      ...
      const successfulGenerated = generationEvents.filter((e: AnalyticsEvent) => e.data.success).length
      const audioSuccessRate = totalGenerated > 0 ? (successfulGenerated / totalGenerated) * 100 : 0

      // Playback metrics
      const playEvents = playbackEvents.filter((e: AnalyticsEvent) => e.data.action === 'play')
      const completeEvents = playbackEvents.filter((e: AnalyticsEvent) => e.data.action === 'complete')
  ...
  async getFavoriteMetrics(): Promise<any> {
    try {
      ...
      const addEvents = events.filter((e: AnalyticsEvent) => e.data.action === 'add')
      const removeEvents = events.filter((e: AnalyticsEvent) => e.data.action === 'remove')
    ...
    try {
      ...
      return {
        ...
        errorTypes: Object.entries(errorTypes)
          .map(([type, count]: [string, unknown]) => ({ type, count: count as number }))
          .sort((a: { type: string; count: number }, b: { type: string; count: number }) => b.count - a.count)
      }
    } catch (error) {
  ...
  getEventsFromStorage(eventType: string | null = null, timeRange: string = '30d'): AnalyticsEvent[] {
    try {
      const events = JSON.parse(localStorage.getItem('analytics_events') || '[]')

      // Filter by time range
      const cutoffTime = this.getTimeRangeCutoff(timeRange)
      let filteredEvents = events.filter((event: AnalyticsEvent) => event.timestamp >= cutoffTime)

      // Filter by event type if specified
      if (eventType) {
        filteredEvents = filteredEvents.filter((event: AnalyticsEvent) => event.type === eventType)
      }
  ...
  async getUsageOverview(): Promise<any> {
    try {
      ...
      const sessions = Array.from(new Set(allEvents.map((e: AnalyticsEvent) => e.sessionId))).length
