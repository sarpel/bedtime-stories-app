...
function App() {
  ...
  const generateAudio = async () => {
    console.log(
      `🎵 [TTS Debug] generateAudio called - story length: ${story?.length || 0}, currentStoryId: ${currentStoryId}`,
    );
    console.log(
  ...
  const saveStory = async (
    ...
    try {
      ...
      console.log(
        `🎵 [Save Pipeline] ${isAutoSave ? "Auto-saving" : "Manual saving"} story...`,
      );
      ...
      console.log(
        `🎵 [Save Pipeline] ${isAutoSave ? "Auto-save" : "Manual save"} completed:`,
        newId,
      );
