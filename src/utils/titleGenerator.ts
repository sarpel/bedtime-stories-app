...
export function getStoryTitle(story: { story_text?: string; story?: string; custom_topic?: string; customTopic?: string }) {
  if (!story) return ''
...
export async function generateTitleWithLLM(storyText: string, fetcher: (url: string, options: any) => Promise<Response>) {
  if (!storyText) return ''
  ...
  try {
    const res = await fetcher('/api/llm', {
      ...
      headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ provider: 'openai', modelId: 'gpt-5-nano', prompt, max_tokens: 24 })
    })
    if (!res.ok) return ''
