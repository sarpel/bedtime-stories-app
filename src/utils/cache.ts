...
class Cache {
  ...
  constructor(maxSize: number = 100) { // Daha fazla öğe saklayalım
    this.cache = new Map()
  ...
  set(key: string, value: any, ttl: number = 3600000): void { // Varsayılan 1 saat TTL
  ...
  cleanup(): void {
    ...
    const expiredItems = entries.filter(([, item]: [string, CacheItem]) =>
      now - item.timestamp > item.ttl
    )
    expiredItems.forEach(([key]: [string, CacheItem]) => this.cache.delete(key))
    ...
    if (this.cache.size > this.maxSize) {
      ...
      const scored = remainingEntries.map(([key, item]: [string, CacheItem]) => ({
        key,
...
export const storyCache = {
  ...
  createKey(storyType: string, customTopic: string | null, settings: StorySettings): string {
  ...
  // Masalı önbellekle
  setStory(storyType: string, customTopic: string | null, settings: StorySettings, story: string): void {
  ...
  getStory(storyType: string, customTopic: string | null, settings: StorySettings): string | null {
    const key = this.createKey(storyType, customTopic, settings)
    return storyContentCache.get(key)
  },

  // Masal önbellekte var mı kontrol et
  hasStory(storyType: string, customTopic: string | null, settings: StorySettings): boolean {
    const key = this.createKey(storyType, customTopic, settings)
...
export const audioCache = {
  ...
  setAudio(text: string, voiceId: string, settings: AudioSettings, audioUrl: string): void {
  ...
  getAudio(text: string, voiceId: string, settings: AudioSettings): string | null {
    const key = this.createKey(text, voiceId, settings)
