...
jest.unstable_mockModule('./safeLocalStorage.js', () => ({
  default: {
    cleanup: cleanupMock,
  },
}))
...
const listeners = {}
const addEventListenerMock = jest.fn((type, cb) => {
...
global.navigator = { userAgent: 'jest-test-agent', onLine: true }
...
global.performance = { memory: perfMemory }

// Import the module under test after mocks
const mod = await import('./stabilityMonitor.js')
const stabilityMonitor = mod.default
...
describe('handleError and error storage/recovery', () => {
  ...
  it('increments errorCount, logs error with details, stores limited recent errors', () => {
    ...
    for (let i = 0; i < 7; i++) {
      stabilityMonitor.handleError('javascript_error', `msg ${i}`, { foo: i })
    }
