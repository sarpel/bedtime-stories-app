...
const safeLocalStorage = {
  ...
  set: (key: string, value: any) => {
    try {
      ...
      if ((error as Error).name === 'QuotaExceededError') {
        logger.info('localStorage quota exceeded, attempting cleanup', 'STORAGE', {})
  ...
  cleanup: () => {
    try {
      ...
      logger.info(`localStorage cleanup completed, removed ${removedCount} items`, 'STORAGE', { removedCount })
    } catch (error) {
  ...
  emergencyCleanup: () => {
    try {
      ...
      logger.info(`Emergency cleanup completed, removed ${removedCount} items`, 'STORAGE', { removedCount })
      return removedCount
    } catch (error) {
