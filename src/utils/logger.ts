...
class Logger {
  ...
  parseLogLevel(level: string): number {
    ...
    return LOG_LEVELS[levelUpper as LogLevel] !== undefined ? LOG_LEVELS[levelUpper as LogLevel] : LOG_LEVELS.INFO
  }
  ...
  formatMessage(level: string, message: string, context: string | null): string {
    const timestamp = new Date().toISOString()
    const ctx = context || this.context
    return `[${timestamp}] [${level}] [${ctx}] ${message}`
  }

  log(level: string, message: string, context: string | null = null, data: any = null): void {
