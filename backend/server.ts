...
// Constants
const LLM_REQUEST_TIMEOUT_MS = 90000; // 2 minutes
...
${prompt}`;
    ...
    endpoint =
      clientEndpoint ||
      `${geminiBase}/${encodeURIComponent(
        effectiveModel
      )}:generateContent?key=${encodeURIComponent(GEMINI_LLM_API_KEY)}`;
...
app.post("/api/stories", (req, res) => {
  try {
    ...
    if (typeof storyText !== "string" || storyText.trim().length < 5) {
      return res
        .status(400)
        .json({ error: "Masal metni en az 5 karakter olmalıdır." });
    }
  ...
  try {
    const execOnce = async () => {
      if (provider === "elevenlabs") {
        ...
        endpoint =
          clientEndpoint ||
          `${ELEVEN_BASE}/${encodeURIComponent(
            effectiveVoice
        ...
        endpoint =
          ...
          `${base}/${encodeURIComponent(
            effectiveModel
          )}:generateContent?key=${encodeURIComponent(GEMINI_TTS_API_KEY)}`;
        ...
        logger.error({
          ...
          response: JSON.stringify(resp.data).substring(0, 10000),
        });
      ...
      try {
        ...
        logger.error({
          ...
          response: err?.response?.data
            ? JSON.stringify(err.response.data).substring(0, 5000)
            : undefined,
...
app.post("/api/voice-assistant", async (req, res) => {
  ...
  try {
    logger.info({
      ...
      transcript: req.body.transcript?.substring(0, 1000),
    });
...
app.post("/api/raspberry-audio/play", async (req, res) => {
  ...
  try {
    ...
    const omxCommand = `omxplayer --vol ${
      (volume - 100) * 60
    } -o alsa:hw:1,0 "${fullAudioPath}"`;
    ...
    for (let i = 0; i < count; i++) {
      try {
        ...
        const prompt = `${systemPrompt}\n\nMasal türü: ${
          storyTypeMap[selectedType] || selectedType
        }`;
...
if (require.main === module) {
  const HOST = process.env.HOST || "0.0.0.0"; // Tüm network interface'lerinden erişim
